...
def WPAConnect(ssid, psk):
    ...
    print(ssid)
    print(psk)
    CLI = subprocess.Popen(["sudo", "wpa_cli"], stdin = subprocess.PIPE)
    time.sleep(30)
    CLI.stdin.write(bytes(("set_network 0 ssid " + ssid +" \n"),"utf-8"))
    time.sleep(10)
    CLI.stdin.write(bytes(("set_network 0 psk " +psk + " \n"),"utf-8"))
    time.sleep(10)
    CLI.stdin.write(bytes(("reconnect \n"),"utf-8"))
    time.sleep(10)
    CLI.communicate(bytes(("quit \n"),"utf-8"))
    time.sleep(10)
...
#Kill interfering processes
    ...
    #Test Connection
        ...
        #Do stuff to send remote destop info
                Connected == True
                sys.exit()
            #Manipulate Signal value to iterate   
            Signals[numpy.argmin(Signals)] = 100 + NetNum
            NetNum = NetNum + 1
